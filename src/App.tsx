...
export default function App() {
  ...
  const toggleAiContext = (path: string) => {
  ...
  // Helper to fetch file content for AI prompts
  const fetchFileContentForAi = async (path: string) => {
  ...
  // Helper to get directory tree structure
  const fetchRepoTreeForAi = async () => {
    if (!selectedRepo || !selectedBranch) return '';
    const [owner, repo] = selectedRepo.split('/');
    try {
      const refData = await apiRequest(`https://api.github.com/repos/${owner}/${repo}/git/ref/heads/${encodeURIComponent(selectedBranch)}`);
      const commitData = await apiRequest(`https://api.github.com/repos/${owner}/${repo}/git/commits/${refData.object.sha}`);
      const treeData = await apiRequest(`https://api.github.com/repos/${owner}/${repo}/git/trees/${commitData.tree.sha}?recursive=1`);
      if (treeData.tree) {
        return treeData.tree.map((i: any) => i.path).join('\n');
      }
    } catch (_) {}
    return '';
  };
  ...
  const handleAiJudgeComplexity = async () => {
    ...
    try {
      let contextStr = '';
      if (aiContextFiles.size > 0) {
        const contexts = await Promise.all(Array.from<string>(aiContextFiles).map(async p => `File: ${p}\n\`\`\`\n${await fetchFileContentForAi(p)}\n\`\`\``));
        contextStr = `\n\nAdditional Context Files:\n${contexts.join('\n\n')}`;
      }

      const prompt = `Here is the code file (${selectedFilePath}) to analyze:

${editorContent}${contextStr}`;
  ...
  const handleSendConsoleQuery = async (e?: React.FormEvent) => {
    ...
    try {
      const repoTree = await fetchRepoTreeForAi();
      let contextStr = '';
      if (aiContextFiles.size > 0) {
        const contexts = await Promise.all(Array.from<string>(aiContextFiles).map(async p => `File: ${p}\n\`\`\`\n${await fetchFileContentForAi(p)}\n\`\`\``));
        contextStr = `\n\nAdditional Context Files:\n${contexts.join('\n\n')}`;
      ...
      const prompt = `Repository structure:\n${repoTree}\n\nCurrently loaded file: ${selectedFilePath}
Content:
\`\`\`
${editorContent}
\`\`\`${contextStr}
...
Return ONLY the markdown README contents. Do NOT include any conversational introduction or notes.`;
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    if (files.length > 0) {
      // Auto-strip root folder if all files share it
      const parts = files[0].path.split('/');
      if (parts.length > 1) {
        const rootFolder = parts[0] + '/';
        const allShareRoot = files.every(f => f.path.startsWith(rootFolder));
        if (allShareRoot) {
          for (let i = 0; i < files.length; i++) {
            files[i].path = files[i].path.substring(rootFolder.length);
          }
        }
      }
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-slate-950 overflow-hidden relative">
        ...
        <footer className="w-full text-center py-4 px-6 text-[10px] text-slate-500 dark:text-slate-500 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 shrink-0">
          <p>
            Developed by Siyabonga B Phakathi of The Voice & Eye of Bhambatha Inc.
            <br className="mt-1" />
            <a href="https://www.bhambathablog.wordpress.com" target="_blank" rel="noopener noreferrer" className="hover:text-purple-500 transition-colors">Blog</a> |{' '}
            <a href="https://www.facebook.com/C.Datoxic.P" target="_blank" rel="noopener noreferrer" className="hover:text-purple-500 transition-colors">Facebook</a> |{' '}
            <a href="https://www.websim.com/@whisperinggalaxyd" target="_blank" rel="noopener noreferrer" className="hover:text-purple-500 transition-colors">WebSim</a> |{' '}
            <a href="https://www.github.com/datoxic0" target="_blank" rel="noopener noreferrer" className="hover:text-purple-500 transition-colors">GitHub</a> |{' '}
            <a href="https://discord.com/channels/datoxic0" target="_blank" rel="noopener noreferrer" className="hover:text-purple-500 transition-colors">Discord</a> |{' '}
            <a href="https://x.com/Siya_B_Phakathi" target="_blank" rel="noopener noreferrer" className="hover:text-purple-500 transition-colors">X</a>
          </p>
        </footer>
      </main>
